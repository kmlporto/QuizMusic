...
const geraLetra = (artista, musica) => {
...
async function perguntaCompleteLetra (artMus) {
	return artMus.then(v => {
		...
		const letraMusica = geraLetra (v[respostaCorretaPosicao].artUrl, v[respostaCorretaPosicao].musDesc)
...
export function geraPerguntas (artMus, artistaSelecionado, numeroPerguntas) {
	let perguntas = {
	...
	for (let i = 1; i <= numeroPerguntas; i++) {
		...
		if (artistSelecionado === 'vazio') {
			...
			switch (random) {
				case 1:
					arrayPerguntas.push(perguntaCompleteLetra (artMus))
					break;

				case 2:
					arrayPerguntas.push(perguntaNomeArtista (artMus))
					break;
			}
		} else {
			let random = getRandomInt(1,3)
			switch (random) {
				case 1:
					arrayPerguntas.push(perguntaCompleteLetra (filtroArtista(artMus, artistaSelecionado)))
					break;

				case 2:
					arrayPerguntas.push(perguntaNomeMus (filtroArtista(artMus, artistaSelecionado)))
					break;
			}
		}
	}
...
export function exibePergunta (arrayPerguntas, posicaoPergunta) {
	return arrayPerguntas[posicaoPergunta]
}
